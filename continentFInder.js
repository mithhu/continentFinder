let continent = { //see https://gist.github.com/nobuti/3816985
...
let arr = [[29.8500, 31.3333], [34.0522, 118.2437]
]

let continentArray = [];
async function getCountryName(lat, lon) {
  let response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lon}&key=AIzaSyB94wtYbbU7JEsO8XIF9sJpMTAlBsBVti0`)
  let fetchCountryInfo = await response.json()
  let countryName = await fetchCountryInfo.results[0].address_components.filter(cntr => cntr.types[0] === "country")[0].short_name
  continentArray.push(continent[countryName])
}

let result = arr.map(arr => getCountryName(arr[0], arr[1]))
